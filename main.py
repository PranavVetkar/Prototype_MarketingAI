...
# Define Default Admin User (Hardcoded)
DEFAULT_ADMIN_EMAIL = "admin@demo.com"
DEFAULT_ADMIN_PASSWORD = "password123"
DEFAULT_ADMIN_UID = "DEMO_UID_001" # A placeholder UID
...
class GenerateTaskRequest(BaseModel):
    # UID is still required by the frontend, but is the hardcoded DEMO_UID
    uid: str
    prompt: str
    audience: str
    image_base64: Optional[str] = None

# --- FRONTEND ROUTE (Serve index.html directly) ---
...
async def api_login(data: LoginRequest):
    ...
    if data.email == DEFAULT_ADMIN_EMAIL and data.password == DEFAULT_ADMIN_PASSWORD:
        ...
        # Note: We combine Register/Update Password functionality here for simplicity
...
async def api_generate_task(data: GenerateTaskRequest):
    ...
    return JSONResponse({
        ...
        "task_id": "DEMO_TASK_" + os.urandom(4).hex(), # Generate a temp ID
